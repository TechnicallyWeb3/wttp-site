...
export async function generateManifestStandalone(
  sourcePath: string,
  ...
  options?: {
    ...
    wttpSite?: Contract; // IBaseWTTPSite contract instance
    chainId?: number;
    chainName?: string;
  ...
  const hasSite = options?.wttpSite !== undefined;
  ...
  let dps: Contract | null = null;
  let dpr: Contract | null = null;
  let gasPrice = ethers.parseUnits("50", "gwei");
  let chainData: Manifest["chainData"];

  if (estimationMode && options?.provider && options?.wttpSite) {
    const provider = options.provider;
    const wttpSite = options.wttpSite;
    ...
    chainData = {
      contractAddress: options.wttpSite ? await options.wttpSite.getAddress() : "0x0000000000000000000000000000000000000000",
  ...
  for (const filePath of allFiles) {
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      if (estimationMode && dpr && options?.wttpSite && options?.provider) {
        // Get royalty
        ...
        try {
          ...
          const estimate = await options.wttpSite.PUT.estimateGas(putRequest, {
            value: royalty
          });
